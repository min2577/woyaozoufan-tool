...
const NAMES_FILE = path.join(__dirname, '../data/dish_names.json');
...
try {
  ...
  console.error('无法读取菜名库，请先运行 seed_names.js');
  process.exit(1);
}
...
console.log(`总菜名数：${allDishNames.length}，已存在：${existingNames.size}，待生成：${tasks.length}`);
...
async function run() {
  let count = 0;
  // 为了演示，我们先跑前 5 个，用户可以自己改这个循环限制
  // 在实际全量运行时，可以去掉 slice 或改为 while 循环
  const BATCH_SIZE = 5; 
